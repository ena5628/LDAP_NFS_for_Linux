...
- isoファイルのダウンロード
- virtualboxで仮想マシンを作成
- 設定のストレージからダウンロードしたisoファイルを設定（空ディスクに設定）

> ここでネットワーク設定をブリッジアダプターに変更する（NAT → ブリッジアダプター）
...
今回の検証を行っているうえで、途中でエラーが出るような無駄な作業を行っていた場面もありましたが、とても勉強になり、理解がより深まったように思います。

最後に、今回の検証で細かい設定までは理解できてなく、AI等を活用して解決したので、自分で考えて問題解決取り組む力を身に着ける必要があると感じました。
また、エラーログ等を読む力も育てていかないといけないと実感しました。


## 参考資料
- [VirtualBoxを用いたLinux環境の構築](https://qiita.com/Horiiii/items/268a1496285f164f85d)
